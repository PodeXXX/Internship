...
class ColorBrain:
    ...
    def search_products_by_color(self, user_input):
        ...
        try:
            ...
            # =======================================================
            # E. [MỚI] TRỘN ĐA DẠNG KẾT QUẢ ĐỂ HIỂN THỊ
            # =======================================================
...
#         # 3. [MỚI] TỪ ĐIỂN LOẠI TRỪ (Negative Filter)
#         # Cấu trúc: 'Từ khóa chính': ['Từ cần loại bỏ nếu user không gõ']
#         self.product_exclusions = {
#             'ghế': ['bao', 'vỏ', 'áo', 'cover', 'case', 'chân', 'leg', 'khung', 'frame'],
#             'sofa': ['bao', 'vỏ', 'áo', 'cover', 'case', 'chân', 'leg', 'khung', 'frame'],
#             'chair': ['cover', 'case', 'leg', 'frame'],
#             'gối': ['bao', 'vỏ', 'áo', 'cover', 'case'], # Tìm gối thì né bao/áo ra
#             'pillow': ['cover', 'case'],
#             'giường': ['khung', 'dát', 'frame', 'slat'],
#             'bed': ['frame', 'slat'],
#             'bàn': ['mặt', 'top', 'chân', 'leg'],
...
#             query = f"SELECT TOP 500 Name, ColorVIE, Code FROM [dbo].[B20ItemHQ] WHERE ColorVIE LIKE ?"
#             df = pd.read_sql(query, conn, params=[f'%{target_color}%'])
#             conn.close()

#             if df.empty: return None

#             # D. Lọc Màu Chính Xác (Logic cũ)
#             def is_true_color_match(row_text):
#                 if not row_text: return False
#                 text_lower = str(row_text).lower()
#                 target_lower = target_color.lower()
...
#                 best_match_in_row = None
#                 for vocab in self.vocab_sorted:
#                     if re.search(f"\\b{re.escape(vocab.lower())}\\b", text_lower):
#                         best_match_in_row = vocab.lower()
#                         break 
...
#             # E. [LOGIC MỚI] Lọc theo Tên Sản Phẩm + Loại Trừ Thông Minh
#             if target_product_kw:
#                 # E1. Lọc lấy dòng chứa từ khóa chính (VD: lấy dòng có "sofa")
#                 df_filtered = df_filtered[df_filtered['Name'].str.contains(target_product_kw, case=False, na=False)]
                
#                 # E2. Lọc bỏ các từ "ký sinh" (Exclusions)
#                 # Lấy danh sách từ cấm cho sản phẩm này (VD: sofa -> cấm [bao, áo, vỏ...])
#                 exclusions = self.product_exclusions.get(target_product_kw, [])
...
#                         for exc in exclusions:
#                             # Nếu trong tên sản phẩm có từ cấm (VD: "Bao ghế sofa")
#                             if exc in name_lower:
#                                 # NHƯNG: Nếu người dùng CHỦ ĐỘNG gõ từ cấm đó (VD: user gõ "Bao sofa")
#                                 # Thì giữ lại (Return True). 
#                                 # Còn nếu user KHÔNG gõ (chỉ gõ "Sofa"), mà tên sp lại có "Bao" -> Loại (Return False)
#                                 if exc not in user_input_lower:
#                                     return False 
#                         return True
...
#                 "keyword": target_color,
#                 "product_filter": target_product_kw,
#                 "products": df_filtered.to_dict('records'),
#                 "count": len(df_filtered)
#             }

#         except Exception as e:
#             print(f"Lỗi ColorBrain: {e}")
#             return None